...
public class SamlSecurityRealm extends SecurityRealm {
    ...
    /**
     * Added to support LDAP Lookup for the logged in user by the IdP
     */
    ...
    //Hard coded JSON to create LDAPSecurityRealm instance --Rohit
    String ldapJson = "{\"realm\":{\"value\":\"2\",\"stapler-class\":\"hudson.security.LDAPSecurityRealm\",\"$class\":\"hudson.security.LDAPSecurityRealm\",\"configurations\":{\"server\":\"ldaps://stldc66.corp.mastercard.test ldaps://stldc68.corp.mastercard.test\",\"rootDN\":\"DC=corp,DC=mastercard,DC=test\",\"inhibitInferRootDN\":false,\"userSearchBase\":\"OU=MasterCard Users\",\"userSearch\":\"sAMAccountName={0}\",\"groupSearchBase\":\"OU=MasterCard Groups\",\"groupSearchFilter\":\"(&(objectCategory=group)(cn={0}))\",\"groupMembershipStrategy\":{\"value\":\"0\",\"attributeName\":\"memberOf\",\"stapler-class\":\"jenkins.security.plugins.ldap.FromUserRecordLDAPGroupMembershipStrategy\",\"$class\":\"jenkins.security.plugins.ldap.FromUserRecordLDAPGroupMembershipStrategy\"},\"managerDN\":\"CN=cddsauth,OU=Generic Accounts,OU=MasterCard Users,DC=corp,DC=mastercard,DC=test\",\"managerPasswordSecret\":\"{AQAAABAAAAAQ/hr56GlvyOn4AcbOSX6+ngfmZ10Dcuwjydi2kmGrZLY=}\",\"$redact\":\"managerPasswordSecret\",\"displayNameAttributeName\":\"displayName\",\"mailAddressAttributeName\":\"mail\",\"ignoreIfUnavailable\":false},\"\":[\"0\",\"0\"],\"userIdStrategy\":{\"stapler-class\":\"jenkins.model.IdStrategy$CaseInsensitive\",\"$class\":\"jenkins.model.IdStrategy$CaseInsensitive\"},\"groupIdStrategy\":{\"stapler-class\":\"jenkins.model.IdStrategy$CaseInsensitive\",\"$class\":\"jenkins.model.IdStrategy$CaseInsensitive\"},\"disableMailAddressResolver\":false,\"disableRolePrefixing\":false},\"testUser\":\"e059704\",\"testPassword\":\"\"}";

    List<String> allRolesForUser;

    private LDAPSecurityRealm realm;
    private Boolean bForceIdPGroupsOverWrite = true;
    ...
    public SamlSecurityRealm(
        ...
        this.ldapLookUp = ldapUserLookUp;
    ...
    public HttpResponse doCommenceLogin(final StaplerRequest request, final StaplerResponse response, @QueryParameter
        ...
        //Creating instance of LDAPSecurityRealm based upon incoming Stapler request
        ...
        JSONObject json = JSONObject.fromObject(new JsonSlurper().parseText(ldapJson));
        //JSONObject jsonTemp = JSONObject.fromObject(new JsonSlurper().parseText(ldapJson));

        bForceIdPGroupsOverWrite = this.advancedConfiguration.getForceLDAPUser();

        //Replace Values
        json.getJSONObject("realm").getJSONObject("configurations").replace("server", this.advancedConfiguration.getServer());
        json.getJSONObject("realm").getJSONObject("configurations").replace("rootDN", this.advancedConfiguration.getRootDN());
        json.getJSONObject("realm").getJSONObject("configurations").replace("inhibitInferRootDN", false);
        json.getJSONObject("realm").getJSONObject("configurations").replace("userSearchBase", this.advancedConfiguration.getUserSearchBase());
        json.getJSONObject("realm").getJSONObject("configurations").replace("userSearch", this.advancedConfiguration.getUserSearch());
        json.getJSONObject("realm").getJSONObject("configurations").replace("groupSearchBase", this.advancedConfiguration.getGroupSearchBase());
        json.getJSONObject("realm").getJSONObject("configurations").replace("groupSearchFilter", this.advancedConfiguration.getGroupSearchFilter());
        //Group membership strategy should remain hard coded to "Parse user attribute list for LDAP groups" with attribute "memberOf"
        json.getJSONObject("realm").getJSONObject("configurations").replace("managerDN", this.advancedConfiguration.getManagerDN());
        json.getJSONObject("realm").getJSONObject("configurations").replace("managerPasswordSecret", this.advancedConfiguration.getManagerPasswordSecret().getEncryptedValue()); //We may need to send the encrypted AES string here
        json.getJSONObject("realm").getJSONObject("configurations").replace("displayNameAttributeName", this.advancedConfiguration.getDisplayNameAttributeName());
        json.getJSONObject("realm").getJSONObject("configurations").replace("mailAddressAttributeName", this.advancedConfiguration.getMailAddressAttributeName());

        JSONObject realmCfg;
        realmCfg = json.getJSONObject("realm");

        // instantiate LDAPSecurityRealm
        realm = request.bindJSON(LDAPSecurityRealm.class, realmCfg);
    ...
    public HttpResponse doFinishLogin(final StaplerRequest request, final StaplerResponse response) {
        ...
        /////////////////////////////////////////////////////////////////////////
        if(bForceIdPGroupsOverWrite) {
            LDAPSecurityRealm.DescriptorImpl descriptor = new LDAPSecurityRealm.DescriptorImpl();
            List<String> lookUpAuthorities = /*(List<String>)*/ descriptor.lookUpPassedUser(realm, username);
            LOG.log(Level.FINEST, "All authorities found for - " + username + " are - " + lookUpAuthorities); //Rohit
            allRolesForUser = new ArrayList<String>();
            allRolesForUser.addAll(lookUpAuthorities);
        }
        else
            LOG.log(Level.FINEST, "SAML Advanced Configuration setting for - Force IdP Assigned Groups Over Write - was not checked. Unable to perform IdP returned roles overwrite"); //Rohit
        //============================================
        /////////////////////////////////////////////////////////////////////////
    ...
    List<GrantedAuthority> loadGrantedAuthorities(SAML2Profile saml2Profile) {
        ...
        try {
            if(allRolesForUser != null) {
                if (!allRolesForUser.isEmpty()) {
                    if(groups == null)
                    {
                        //We probably don't have any assigned groups to Azure AD role - FSSO-Jenkins-Users
                        groups = new ArrayList<>();
                        groups.clear();
                        groups = getListOfValues(allRolesForUser);
                    }
                    else {
                        //Forcing drop of all existing groups fetched from Azure AD
                        //BUGFIX:: Defect#DE288268 - Azure AD Role with No assigned Groups causes exception
                        if(groups.size() <= 0) {
                            groups = getListOfValues(allRolesForUser);
                        }
                        else {
                            groups.clear();
                            groups.addAll(allRolesForUser);
                        }
                    }
                }
                else
                    LOG.log(Level.FINEST, "Unable to overwrite. Roles list from user lookup came back empty - " + allRolesForUser); //Rohit
            }
            else {
                LOG.log(Level.WARNING, "UserLookUp failed. Probably the option - Force IdP Assigned Groups Over Write - wasn't checked!"); //Rohit
            }
        }
        catch(Exception e)
        {
            LOG.log(Level.INFO, "EXCEPTION :: When looking up authenticated users LDAP assigned roles - " + e.getMessage()); //Rohit
        }
    ...
    public SamlPluginConfig getSamlPluginConfig() {
        SamlPluginConfig samlPluginConfig = new SamlPluginConfig(displayNameAttributeName, groupsAttributeName,
                ...
                usernameAttributeName, logoutUrl, binding, encryptionData, advancedConfiguration, ldapLookUp);
    ...
    //Adding property to support LDAP Lookup of the logged in user in SAML configuration
    public SamlLdapLookUp getLdapLookUp() {
        return ldapLookUp;
    }
